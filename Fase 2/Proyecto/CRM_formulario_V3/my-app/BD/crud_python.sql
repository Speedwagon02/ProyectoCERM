...
CREATE TABLE IF NOT EXISTS tbl_contactos (
    ...
    FOREIGN KEY (id_miembro_responsable) REFERENCES tbl_empleados(id_empleado)
...
CREATE TABLE IF NOT EXISTS tbl_tickets (
    ...
    `descripcion_ticket` VARCHAR(255) NOT NULL,
    `tipo_ticket` int DEFAULT NULL,
    `id_user` INT,
    `id_empleado_asignado` INT,
    FOREIGN KEY (id_empleado_asignado) REFERENCES tbl_empleados(id_empleado),
...
CREATE TABLE IF NOT EXISTS tbl_ventas (
    ...
    `empresa` VARCHAR(255),
    `fecha_cobro` DATE,
    `fecha_venta_vencimiento` DATE,
    FOREIGN KEY (id_cliente) REFERENCES tbl_contactos(id_contacto)
...
CREATE TABLE IF NOT EXISTS Informes (
    ...
    id_generado_por INT,
    contenido TEXT,
    FOREIGN KEY (id_generado_por) REFERENCES tbl_empleados(id_empleado)
);
