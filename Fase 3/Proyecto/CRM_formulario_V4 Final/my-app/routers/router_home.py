...
@app.route('/generar_correo', methods=['GET'])
def viewCorreo():
    if 'conectado' in session:
        # Redirigir a la URL externa cuando el usuario esté conectado
        return redirect('https://9f36-190-47-215-206.ngrok-free.app/')
    else:
...
def enviar_correo():
    try:
        ...
        # Si el usuario seleccionó enviar su propio mensaje
        if tipoMensaje == 'mensajePropio':
            msg.attach(MIMEText(mensaje, 'plain'))  # Usamos 'plain' porque es texto simple

        # Si el usuario seleccionó enviar el contenido HTML
        elif tipoMensaje == 'mensajeHTML':
            with open('body_correo.html', 'r') as archivo:
                html = archivo.read()
            msg.attach(MIMEText(html, 'html'))  # Usamos 'html' para el cuerpo del correo en formato HTML
        ...
        return jsonify({'mensaje': f'Error al enviar el correo: {str(e)}'})
